...
static uint32_t crc32(std::string name)
{
	...
	while (unHashedLength >= 8)
	{
		...
		crc_Hash =
			...
			crc_table_48[(second >> 8) & 0xFF] ^
			crc_table_56[second & 0xFF] ^
			crc_table_64[(first >> 24) & 0xFF] ^
			crc_table_72[(first >> 16) & 0xFF] ^
			crc_table_80[(first >> 8) & 0xFF] ^
			crc_table_88[first & 0xFF];
		unHashedLength -= 8;
	}
	...
	if (unHashedLength >= 4)
	{
		...
		crc_Hash =
			...
			crc_table_40[(first >> 16) & 0xFF] ^
			crc_table_48[(first >> 8) & 0xFF] ^
			crc_table_56[first & 0xFF];
		unHashedLength -= 4;
	}
...
static constexpr uint32_t crc_tableCT[256] = {
	0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
	0x0EDB8832, 0x79DCB8A4, 0xE0D5E91E, 0x97D2D988, 0x09B64C2B, 0x7EB17CBD, 0xE7B82D07, 0x90BF1D91,
	0x1DB71064, 0x6AB020F2, 0xF3B97148, 0x84BE41DE, 0x1ADAD47D, 0x6DDDE4EB, 0xF4D4B551, 0x83D385C7,
	0x136C9856, 0x646BA8C0, 0xFD62F97A, 0x8A65C9EC, 0x14015C4F, 0x63066CD9, 0xFA0F3D63, 0x8D080DF5,
...
constexpr uint32_t crc32CT(const char * string)
{
	return (crc32CT<index - 1>(string) >> 8) ^ crc_tableCT[(crc32CT<index - 1>(string) ^ string[index]) & 0x000000FF];
}
template<>
constexpr uint32_t crc32CT<size_t(-1)>(const char * dtring)
{
	return 0xFFFFFFFF;
}
...
#else
#define UID(x) (crc32CT<sizeof(x) - 2>(x) ^ 0xFFFFFFFF)
